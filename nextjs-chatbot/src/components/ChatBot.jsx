...
function ChatBot() {
  ...
  useEffect(() => {
    ...
    console.log("ChatBot: Token found:", !!token);
    if (token) {
      // Since we're already authenticated (passed from parent), just fetch messages
  ...
  const handleSubmit = async () => {
    ...
    if (!schemaSubmitted) {
      ...
      try {
        ...
        const res = await api.post("/api/generate-query", {
          database,
          schema,
          prompt: input,
        ...
        const aiMessage = {
          sender: "bot",
          text: res.data.query || "No response from AI.",
          timestamp: new Date().toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
        ...
        let errorText = "❌ Failed to generate query. Please try again or check your connection.";
  ...
  // Remove the authentication check since it's handled by the parent component
